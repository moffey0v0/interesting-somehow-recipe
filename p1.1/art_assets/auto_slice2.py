...
def is_background(pixel):
    # Depending on mode, pixel can be an int or a tuple
    if isinstance(pixel, int): # L mode or P
        return pixel > 245
...
def get_bounding_boxes(img):
    ...
    # Group boxes by row. We assume a roughly grid-like structure.
    # We sort by Y, then group if they overlap significantly in Y.
    ...
    for b in boxes:
        if not rows:
            ...
            if abs(b[1] - last_y) < 40: # threshold for same row
                rows[-1].append(b)
            else:
                rows.append([b])
...
def make_transparent(img, padding=10):
    ...
    # Simple color replacement for white background to transparent
    for y in range(height):
        for x in range(width):
...
OUTPUT_DIR = os.path.join(ASSETS_DIR, 'icons_auto2')
